          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">{paragraphs.map(p => (
            <div key={p.id} className="p-6 rounded-2xl relative transition-all group border-2 border-slate-200 bg-white hover:border-blue-500 hover:shadow-xl"><button type="button" onClick={() => setParagraphs(prev => prev.filter(x => x.id !== p.id))} className="absolute top-4 right-4 p-2 text-slate-400 hover:text-red-600 transition-all"><Trash2 size={20} /></button>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-1"><label className="text-[10px] font-black text-blue-600 uppercase tracking-widest pl-1">জড়িত</label><input type="text" className="w-full h-11 px-3 border border-slate-200 rounded-xl text-right font-black bg-slate-50 outline-none focus:border-blue-400" value={rawInputs[`${p.id}-involvedAmount`] || toBengaliDigits(p.involvedAmount)} onChange={e => handleNumericInputChange(p.id, 'involvedAmount', e.target.value)} /></div>
                <div className="space-y-1"><label className="text-[10px] font-black text-emerald-600 uppercase tracking-widest pl-1">আদায়</label><input type="text" className="w-full h-11 px-3 border border-slate-200 rounded-xl text-right font-black bg-slate-50 outline-none focus:border-emerald-400" value={rawInputs[`${p.id}-recoveredAmount`] || toBengaliDigits(p.recoveredAmount)} onChange={e => handleNumericInputChange(p.id, 'recoveredAmount', e.target.value)} /></div>
                <div className="space-y-1"><label className="text-[10px] font-black text-indigo-600 uppercase tracking-widest pl-1">সমন্বয়</label><input type="text" className="w-full h-11 px-3 border border-slate-200 rounded-xl text-right font-black bg-slate-50 outline-none focus:border-indigo-400" value={rawInputs[`${p.id}-adjustedAmount`] || toBengaliDigits(p.adjustedAmount)} onChange={e => handleNumericInputChange(p.id, 'adjustedAmount', e.target.value)} /></div>
              </div>
            </div>
          ))}</div>
        </div>
...
const SettlementTable = ({ entries, onDelete, onEdit, designMode }) => {
  ...
  const tdMoney = "border border-slate-300 px-0.5 py-1 text-right align-middle text-[11px] md:text-[12px] font-black text-slate-900";
    ...
    <div className="space-y-4 w-full">
      ...
      <div className="table-container">
        <table>
          ...
          <tfoot>
            <tr><td colSpan={3} className={grandLabelStyle}>সর্বমোট (রিপোর্ট) | মীমাংসিত: {toBengaliDigits(grandTotals.paraCount)}</td><td className={grandValueStyle}>-</td><td className={grandValueStyle}>-</td><td className={grandValueStyle + " text-right text-amber-400"} data-type="amount">{toBengaliDigits(Math.round(grandTotals.inv))}</td><td className={grandValueStyle}>-</td><td className={grandValueStyle} data-type="amount">{toBengaliDigits(Math.round(grandTotals.vR))}</td><td className={grandValueStyle} data-type="amount">{toBengaliDigits(Math.round(grandTotals.vA))}</td><td className={grandValueStyle} data-type="amount">{toBengaliDigits(Math.round(grandTotals.iR))}</td><td className={grandValueStyle} data-type="amount">{toBengaliDigits(Math.round(grandTotals.iA))}</td><td className={grandValueStyle} data-type="amount">{toBengaliDigits(Math.round(grandTotals.oR))}</td><td className={grandValueStyle} data-type="amount">{toBengaliDigits(Math.round(grandTotals.oA))}</td><td className={grandValueStyle + " text-amber-400"} data-type="amount">{toBengaliDigits(Math.round(grandTotals.tRec))}</td><td className={grandValueStyle + " text-amber-400"} data-type="amount">{toBengaliDigits(Math.round(grandTotals.tAdj))}</td></tr>
          </tfoot>
        </table>
      </div>
    </div>
              ...
              {reportData.map(m => m.entityRows.map((r, ri) => {
                ...
                return (
                  <tr key={r.entity}>
                    ...
                    <td className={tdS}>{toBengaliDigits(r.prev.unsettledCount)}</td><td className={tdS + " text-right"} data-type="amount">{toBengaliDigits(Math.round(r.prev.unsettledAmount))}</td>
                    <td className={tdS}>{toBengaliDigits(r.currentRaisedCount)}</td><td className={tdS + " text-right"} data-type="amount">{toBengaliDigits(Math.round(r.currentRaisedAmount))}</td>
                    <td className={tdS}>{toBengaliDigits(tUC)}</td><td className={tdS + " text-right"} data-type="amount">{toBengaliDigits(Math.round(tUA))}</td>
                    <td className={tdS}>{toBengaliDigits(r.prev.settledCount)}</td><td className={tdS + " text-right"} data-type="amount">{toBengaliDigits(Math.round(r.prev.settledAmount))}</td>
                    <td className={tdS}>{toBengaliDigits(r.currentSettledCount)}</td><td className={tdS + " text-right"} data-type="amount">{toBengaliDigits(Math.round(r.currentSettledAmount))}</td>
                    <td className={tdS}>{toBengaliDigits(tSC)}</td><td className={tdS + " text-right"} data-type="amount">{toBengaliDigits(Math.round(tSA))}</td>
                    <td className={tdS + " bg-amber-50"}>{toBengaliDigits(tUC - tSC)}</td><td className={tdS + " text-right bg-amber-50"} data-type="amount">{toBengaliDigits(Math.round(tUA - tSA))}</td>
                  </tr>
                );
              }))}
            </tbody>
            <tfoot>
              <tr className="bg-slate-900 text-white font-black"><td colSpan={2} className="p-2 text-center text-[11px]">সর্বমোট (শাখা)</td><td className="p-2 text-center text-[11px]">{toBengaliDigits(grandTotals.pUC)}</td><td className="p-2 text-right text-[11px]" data-type="amount">{toBengaliDigits(Math.round(grandTotals.pUA))}</td><td className="p-2 text-center text-[11px]">{toBengaliDigits(grandTotals.cRC)}</td><td className="p-2 text-right text-[11px]" data-type="amount">{toBengaliDigits(Math.round(grandTotals.cRA))}</td><td className="p-2 text-center text-[11px]">{toBengaliDigits(grandTotals.pUC + grandTotals.cRC)}</td><td className="p-2 text-right text-[11px]" data-type="amount">{toBengaliDigits(Math.round(grandTotals.pUA + grandTotals.cRA))}</td><td className="p-2 text-center text-[11px]">{toBengaliDigits(grandTotals.pSC)}</td><td className="p-2 text-right text-[11px]" data-type="amount">{toBengaliDigits(Math.round(grandTotals.pSA))}</td><td className="p-2 text-center text-[11px]">{toBengaliDigits(grandTotals.cSC)}</td><td className="p-2 text-right text-[11px]" data-type="amount">{toBengaliDigits(Math.round(grandTotals.cSA))}</td><td className="p-2 text-center text-[11px]">{toBengaliDigits(grandTotals.pSC + grandTotals.cSC)}</td><td className="p-2 text-right text-[11px]" data-type="amount">{toBengaliDigits(Math.round(grandTotals.pSA + grandTotals.cSA))}</td><td className="p-2 text-center text-[11px] bg-amber-600">{(grandTotals.pUC + grandTotals.cRC) - (grandTotals.pSC + grandTotals.cSC)}</td><td className="p-2 text-right text-[11px] bg-amber-600" data-type="amount">{toBengaliDigits(Math.round((grandTotals.pUA + grandTotals.cRA) - (grandTotals.pSA + grandTotals.cSA)))}</td></tr>
            </tfoot>
          </table>
        </div>
      </div>
