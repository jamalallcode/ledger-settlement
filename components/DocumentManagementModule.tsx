...
export const DocumentManagementModule: React.FC<DocumentManagementModuleProps> = ({
  ...
  // Auto scroll to top when Jaripatra view is opened
  useEffect(() => {
    if (showJaripatraView) {
      window.scrollTo({ top: 0, left: 0, behavior: "instant" });
      document.documentElement.scrollTop = 0;
      document.body.scrollTop = 0;
      const el = document.getElementById("jaripatra-top-anchor");
      if (el) {
          ...
          <div className="sticky top-0 z-40 bg-slate-900 text-white rounded-none border border-slate-700 shadow-md px-3 py-1.5 flex flex-wrap items-center justify-between gap-2 no-print">
            ...
            <div className="flex items-center flex-wrap gap-1.5">
              ...
              <button
                ...
                title="MS Word বা নথিতে সরাসরি পেস্ট করার জন্য              {/* 6. Official Table (Clean Government Format) */}
              <div className="pt-2">
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse border-2 border-black text-xs sm:text-[12px]">
                    <thead>
                      ...
                      <tr className="bg-slate-50 font-bold border-b border-black text-center">
                        {jaripatraColumns.map((col) => (
                          <th
                            key={col.id}
                            className={`border border-black p-1.5 text-center ${col.width || ''}`}
                          >
                            <input
                              type="text"
                              ...
                              className={`w-full text-center font-bold text-black bg-transparent outline-none ${
                                isJaripatraEditable
                                  ? "hover:bg-slate-100 focus:bg-blue-50"
                    ...
                    {/* Table Body - Clean Official Cells (No Floating Buttons/Handles) */}
                    <tbody>
