...
import { addMonths, format as dateFnsFormat, endOfDay } from 'date-fns';
...
// Fixed: The entire file was corrupted with escaped backslashes and quotes causing syntax failure.
// Removed all \" and restored proper JSX structure.
        ...
        <div className="absolute top-[55px] right-0 w-64 bg-white border border-slate-200 rounded-2xl shadow-2xl z-[500] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="p-2 max-h-[350px] overflow-y-auto no-scrollbar">
            {Array.from({length: 24}).map((_, i) => {
              ...
              return (
                <div key={i} onClick={() => { setSelectedCycleDate(d); setIsCycleDropdownOpen(false); }} className={`px-4 py-2.5 mb-1 rounded-xl cursor-pointer transition-all ${cycle.label === activeCycle.label ? 'bg-blue-600 text-white shadow-lg' : 'hover:bg-slate-50 text-slate-700 font-bold'}`}>
  ...
  return (
    <div id="section-report-summary" className="space-y-4 py-2 w-full animate-report-page relative">
      ...
      <div id="summary-header-controls" className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm no-print relative">
        ...
        <div className="flex flex-wrap items-center gap-4"><HistoricalFilter /><button onClick={() => window.print()} className="h-[44px] px-6 bg-slate-900 text-white rounded-xl font-black text-sm flex items-center gap-2 hover:bg-black transition-all shadow-lg active:scale-95"><Printer size={18} /> প্রিন্ট</button></div>
      </div>
      ...
      {isAdmin && (
        <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200 flex flex-col md:flex-row items-center gap-6 no-print animate-in slide-in-from-left duration-1000 shadow-sm mt-6">
          ...
          <div className="px-8 py-3 bg-slate-50 rounded-2xl border border-slate-200 shadow-inner flex flex-col items-end gap-4">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Calculated Status</span>
            <span className="text-[12px] font-black text-emerald-600 flex items-center gap-2">
              ACCURATE & SYNCED <CheckCircle2 size={16} />
            </span>
          </div>
        </div>
      )}
