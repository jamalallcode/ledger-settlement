...
import { ChevronLeft, Printer, Database, Settings2, CheckCircle2, CalendarDays, ChevronDown, Check, LayoutGrid, PieChart, Search, CalendarSearch, X, Lock, KeyRound, Pencil, Unlock, Mail, Send, FileEdit, AlertCircle, BarChart3 } from 'lucide-react';
...
const ReturnView: React.FC<ReturnViewProps> = ({ 
        ...
        if (entry.paragraphs) {
          entry.paragraphs.forEach(p => {
            ...
            if (p.id && !processedParaIds.has(p.id) && hasDigit) {
              ...
              // CRITICAL FIX: Only Full Settlement counts in recursive opening to prevent discrepancy
          ...
          matchingEntries.forEach(entry => {
            ...
            if (entry.paragraphs) {
              entry.paragraphs.forEach(p => {
                ...
                if (p.id && !processedParaIds.has(p.id) && hasDigit) {
                  processedParaIds.add(p.id);
                  // CORE FIX: Excluding 'Partial' from Count and Settled Amount to satisfy "2 paras and 37253 TK" requirement
                  if (p.status === 'পূর্ণাঙ্গ') {
                      curSC++;
                      curFC++;
                      ...
                      curPC++;
                      // Partial recovery is tracked for internal logic but NOT added to Settled Count/Amount in this summary
                  }
                }
              });
            }
  ...
  const grandTotals = useMemo(() => {
    ...
    return reportData.reduce((acc, mGroup) => {
      mGroup.entityRows.forEach(row => {
        ...
        acc.cFC += (row.fullCount || 0); 
        acc.cPC += (row.partialCount || 0);
      });
      return acc;
        ...
        <div className="absolute top-[55px] right-0 w-64 bg-white border border-slate-200 rounded-2xl shadow-2xl z-[500] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="p-2 max-h-[350px] overflow-y-auto no-scrollbar">
            ...
            {cycleOptions.map((opt, idx) => (
              <div key={idx} onClick={() => { setSelectedCycleDate(opt.date); setIsCycleDropdownOpen(false); }} className={`flex items-center justify-between px-4 py-2.5 mb-1 rounded-xl cursor-pointer transition-all ${dateFnsFormat(opt.date, 'yyyy-MM') === dateFnsFormat(selectedCycleDate, 'yyyy-MM') ? 'bg-blue-600 text-white shadow-lg' : 'hover:bg-slate-50 text-slate-700 font-bold'}`}>
  ...
  return (
    <div className="space-y-6 py-2 w-full animate-premium-page relative">
       ...
       <div className="table-container border border-slate-300 rounded-sm overflow-visible relative bg-white shadow-xl p-1">
          <div className="text-center py-6 bg-white border-b-2 border-slate-100 mb-2">
          ...
          <table className="w-full border-separate table-fixed">
            ...
            <tbody>
              {reportData.map(m => m.entityRows.map((r, ri) => {
                const totalUC = r.prev.unsettledCount + r.currentRaisedCount;
                const totalUA = r.prev.unsettledAmount + r.currentRaisedAmount;
                const totalSC = r.prev.settledCount + r.currentSettledCount;
                const totalSA = r.prev.settledAmount + r.currentSettledAmount;
                ...
                return (
                  <tr key={r.entity} className="group hover:bg-blue-50/30 transition-colors">
                    ...
                    <td className={tdS}>{toBengaliDigits(r.currentRaisedCount)}</td><td className={tdS}>{toBengaliDigits(Math.round(r.currentRaisedAmount))}</td>
                    <td className={tdS + " bg-slate-50/50 font-bold"}>{toBengaliDigits(totalUC)}</td><td className={tdS + " bg-slate-50/50 font-bold"}>{toBengaliDigits(Math.round(totalUA))}</td>
                    ...
                    <td className={tdS + " text-emerald-600 font-black"}>{toBengaliDigits(r.currentSettledCount)}</td><td className={tdS + " text-emerald-600 font-black"}>{toBengaliDigits(Math.round(r.currentSettledAmount))}</td>
                    <td className={tdS + " bg-emerald-50/30 font-bold"}>{toBengaliDigits(totalSC)}</td><td className={tdS + " bg-emerald-50/30 font-bold"}>{toBengaliDigits(Math.round(totalSA))}</td>
                    <td className={tdS + " bg-amber-50/50 text-blue-700 font-bold"}>{toBengaliDigits(closingUC)}</td><td className={tdS + " bg-amber-50/50 text-blue-700 font-bold"}>{toBengaliDigits(Math.round(closingUA))}</td>
                  </tr>
                );
              }))}
            </tbody>
            <tfoot className="sticky bottom-0 z-[100]">
               <tr className="bg-slate-900 text-white font-black h-[45px]">
                  ...
                  <td className="text-center text-[10px] border border-slate-700 bg-slate-800">{toBengaliDigits(grandTotals.pUC + grandTotals.cRC)}</td><td className="text-center text-[10px] border border-slate-700 bg-slate-800">{toBengaliDigits(Math.round(grandTotals.pUA + grandTotals.cRA))}</td>
                  ...
                  <td className="text-center text-[10px] border border-slate-700 bg-emerald-900/50">{toBengaliDigits(grandTotals.pSC + grandTotals.cSC)}</td><td className="text-center text-[10px] border border-slate-700 bg-emerald-900/50">{toBengaliDigits(Math.round(grandTotals.pSA + grandTotals.cSA))}</td>
                  <td className="text-center text-[11px] border border-slate-700 bg-amber-600">{toBengaliDigits((grandTotals.pUC + grandTotals.cRC) - (grandTotals.pSC + grandTotals.cSC))}</td><td className="text-center text-[11px] border border-slate-700 bg-amber-600">{toBengaliDigits(Math.round((grandTotals.pUA + grandTotals.cRA) - (grandTotals.pSA + grandTotals.cSA)))}</td>
               </tr>
            </tfoot>
          </table>
       </div>
