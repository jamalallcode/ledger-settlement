...
const ReturnView: React.FC<ReturnViewProps> = ({ 
  ...
  // Logic from Part 1: Precision calculation for opening balances
        ...
        if (entry.paragraphs) {
          entry.paragraphs.forEach(p => {
            ...
            if (p.id && !processedParaIds.has(p.id) && hasDigit) {
              ...
              if (status === robustNormalize('পূর্ণাঙ্গ')) { pastSC++; }
              pastSA += settledAmt;
            }
          });
        }
          ...
          matchingEntries.forEach(entry => {
            if (entry.paragraphs && entry.paragraphs.length > 0) {
              entry.paragraphs.forEach(p => { 
                ...
                if (p.id && !processedParaIds.has(p.id) && hasDigit) {
                  processedParaIds.add(p.id);
                  if (status === robustNormalize('পূর্ণাঙ্গ')) { curFC++; curSC++; }
                  else if (status === robustNormalize('আংশিক')) { curPC++; }
                  curSA += settledAmt;
                }
              });
            }
  ...
  const HistoricalFilter = () => (
    <div className="relative no-print" ref={dropdownRef}>
      <div onClick={() => setIsCycleDropdownOpen(!isCycleDropdownOpen)} className={`flex items-center gap-3 px-5 h-[48px] bg-white border-2 rounded-xl cursor-pointer transition-all duration-300 hover:border-blue-400 group ${isCycleDropdownOpen ? 'border-blue-600 ring-4 ring-blue-50 shadow-lg' : 'border-slate-200 shadow-sm'}`}>
         ...
         <span className="font-black text-[13.5px] text-slate-800 tracking-tight">{toBengaliDigits(activeCycle.label)}</span>
      ...
      {isCycleDropdownOpen && (
        <div className="absolute top-[55px] right-0 w-64 bg-white border border-slate-200 rounded-2xl shadow-2xl z-[500] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="p-2 max-h-[350px] overflow-y-auto no-scrollbar">
            {addMonths(new Date(), 0) && Array.from({length: 24}).map((_, i) => {
              const d = addMonths(new Date(new Date().getFullYear(), new Date().getMonth(), 1), -i);
              const cycle = getCycleForDate(d);
              return (
                <div key={i} onClick={() => { setSelectedCycleDate(d); setIsCycleDropdownOpen(false); }} className={`px-4 py-2.5 mb-1 rounded-xl cursor-pointer transition-all ${cycle.label === activeCycle.label ? 'bg-blue-600 text-white' : 'hover:bg-slate-50 text-slate-700 font-bold'}`}>
                  <span className="text-[13px]">{toBengaliDigits(cycle.label)}</span>
                </div>
              );
            })}
          </div>
  ...
  const handleSetupPaste = (e: React.ClipboardEvent, startEntity: string, startField: keyof MinistryPrevStats) => {
    ...
    rows.forEach((row, rowOffset) => {
      ...
      cells.forEach((cell, cellOffset) => {
        ...
        newStats[entityName] = { ...(newStats[entityName] || { unsettledCount: 0, unsettledAmount: 0, settledCount: 0, settledAmount: 0 }), [fieldName]: parseBengaliNumber(cell.trim()) };
      });
    });
  ...
  if (selectedReportType === 'চিঠিপত্র সংক্রান্ত মাসিক রিটার্ন: ডিডি স্যারের জন্য।') {
    return <DDSirCorrespondenceReturn entries={correspondenceEntries?.filter(e => { const d = toEnglishDigits(e.diaryDate || ''); return new Date(d) <= endOfDay(new Date(activeCycle.start.getFullYear(), activeCycle.start.getMonth() + 1, 0)) && !((e.issueLetterNo || '').trim() !== '' && (e.issueLetterDate || '').trim() !== '0000-00-00'); }) || []} activeCycle={activeCycle} onBack={() => setSelectedReportType(null)} isLayoutEditable={isLayoutEditable} />;
  }
