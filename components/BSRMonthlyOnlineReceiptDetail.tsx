...
const BSRMonthlyOnlineReceiptDetail: React.FC<BSRMonthlyOnlineReceiptDetailProps> = ({
  ...
  const downloadExcel = () => {
    const table = document.getElementById('table-bsr-monthly-online-detail');
    if (!table) return;
    ...
    const filename = `বিএসআর_অনলাইন_প্রাপ্তি_মাসিক_প্রতিবেদন_${formattedMonth}.xls`;
  ...
  const getMonthYearStringBN = () => {
    const months = ["জানুয়ারি", "ফেব্রুয়ারি", "মার্চ", "এপ্রিল", "মে", "জুন", "জুলাই", "আগস্ট", "সেপ্টেম্বর", "অক্টোবর", "নভেম্বর", "ডিসেম্বর"];
    const month = months[selectedCycleDate.getMonth()];
    const year = toBengaliDigits(selectedCycleDate.getFullYear().toString());
    return `${month}-${year}`;
  };
  ...
  return (
    <div id="bsr-monthly-online-detail-container" className="space-y-5 py-4 w-full animate-report-page relative bg-white p-5 rounded-3xl border border-slate-100 shadow-xl">
      <IDBadge id="bsr-monthly-online-detail-container" />
        ...
        <div className="flex items-stretch h-[44px] w-fit shadow-md select-none rounded-2xl overflow-hidden border border-slate-200/40 shrink-0">
          ...
          <div className="flex flex-col h-full min-w-[240px] justify-center bg-[#1e40af] px-4">
            <span className="text-white font-[950] tracking-tight text-[13px]">
              অনলাইন প্রাপ্তি - বিএসআর মাসিক রিটার্ন
            </span>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-3 no-print">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[13px] sm:text-[14px] font-black text-slate-800 tracking-tight">
            রেসপনসিবল পার্টি হতে অনলাইনে প্রাপ্ত ব্রডশিট জবাবের মাসিক প্রতিবেদন: {toBengaliDigits(dateFnsFormat(startOfMonthDate, 'dd/MM/yyyy'))} হতে {toBengaliDigits(dateFnsFormat(endOfMonthDate, 'dd/MM/yyyy'))}
          </h3>
        </div>
      ...
      <div id="card-bsr-monthly-online-detail-table-container" className="table-container bg-white shadow-inner rounded-none overflow-visible w-full relative">
        ...
        <div className="hidden print:block text-center space-y-2 mb-4">
          <h2 className="text-base font-black text-slate-900 leading-relaxed">
            Responsible Party হতে অনলাইনে ব্রডশিট জবাব প্রাপ্ত পত্রাদির মাসিক প্রতিবেদন: ({toBengaliDigits(dateFnsFormat(startOfMonthDate, 'dd-MM-yyyy'))} খ্রিঃ হতে {toBengaliDigits(dateFnsFormat(endOfMonthDate, 'dd-MM-yyyy'))} খ্রিঃ পর্যন্ত)
          </h2>
          ...
          <table id="table-bsr-monthly-online-detail" className="w-full border-separate border-spacing-0 !table-auto">
            <thead>
              <tr className="bg-slate-50">
                ...
                <th rowSpan={2} className={`${thStyle} w-[100px]`}>প্রাপ্তজবাব (পত্র সংখ্যা)</th>
                <th rowSpan={2} className={`${thStyle} w-[180px]`}>জবাবের স্মারক নং ও তাং</th>
            ...
            <tbody>
              {filteredEntries.length === 0 ? (
                <tr className="bg-white">
                  <td colSpan={10} className="text-center font-black text-slate-900 text-[14px] sm:text-[15px] py-16 border-r border-b border-slate-400 bg-white">
                    Responsible Party হতে অনলাইনে কোন প্রতিবেদন পাওয়া যায়নি।
                  </td>
                </tr>
              ) : (
...
export default BSRMonthlyOnlineReceiptDetail;
