...
/**
 * IDBadge component moved outside to prevent re-creation on every render
 */
const IDBadge = React.memo(({ id, isLayoutEditable }: { id: string; isLayoutEditable?: boolean }) => {
...
interface CorrespondenceTableRowProps {
  entry: CorrespondenceEntry;
  idx: number;
  pending: Partial<CorrespondenceEntry>;
  isAdmin?: boolean;
  handleInlineChange: (entryId: string, field: keyof CorrespondenceEntry, value: any) => void;
  showCycleHeader: boolean;
...
const CorrespondenceTableRow = React.memo(({
  entry, idx, pending, isAdmin, handleInlineChange, showCycleHeader, currentCycleLabel,
  onEdit, onDelete, onApprove, onReject
}: CorrespondenceTableRowProps) => {
        ...
        <td className={tdCls + " relative group/action text-center"}>
           ...
           {isAdmin && (
             <div className="absolute inset-0 bg-white/95 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center gap-1.5 z-10">
                <button onClick={() => onEdit(entry)} className="p-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all shadow-sm"><Pencil size={12} /></button>
                <button onClick={() => onDelete(entry.id)} className="p-1.5 bg-red-50 text-red-600 rounded-lg hover:bg-red-600 hover:text-white transition-all shadow-sm"><Trash2 size={12} /></button>
                {isPendingForApproval && (
                  <>
                    <button onClick={() => onApprove(entry.id)} className="p-1.5 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-600 hover:text-white transition-all shadow-sm"><Check size={12} /></button>
                    <button onClick={() => onReject(entry.id)} className="p-1.5 bg-rose-50 text-rose-600 rounded-lg hover:bg-rose-600 hover:text-white transition-all shadow-sm"><X size={12} /></button>
                  </>
                )}
             </div>
           )}
...
const CorrespondenceTable: React.FC<CorrespondenceTableProps> = ({
  entries, onBack, isLayoutEditable, isAdmin, onEdit, onInlineUpdate, onDelete, onApprove, onReject, showFilters, setShowFilters
}) => {
  ...
  const handleInlineChange = React.useCallback((entryId: string, field: keyof CorrespondenceEntry, value: any) => {
  ...
  }, [pendingChanges, entries, onInlineUpdate]);
  ...
  return (
    <div id="section-correspondence-register" className="w-full space-y-4 animate-premium-page relative">
      <IDBadge id="section-correspondence-register" isLayoutEditable={isLayoutEditable} />
      ...
      <div className="table-container border border-slate-300 rounded-sm overflow-visible relative shadow-xl bg-white max-w-full">
        <IDBadge id="table-correspondence-ledger" isLayoutEditable={isLayoutEditable} />
          ...
          <tbody>
            {filteredEntries.length > 0 ? (() => {
              ...
              return filteredEntries.map((entry, idx) => {
                ...
                const showCycleHeader = currentCycleLabel && currentCycleLabel !== lastCycle;
                if (showCycleHeader) lastCycle = currentCycleLabel;
                ...
                return (
                  <CorrespondenceTableRow
                    ...
                    entry={entry}
                    idx={idx}
                    pending={pendingChanges[entry.id] || {}}
                    isAdmin={isAdmin}
                    handleInlineChange={handleInlineChange}
                    showCycleHeader={showCycleHeader}
              ...
              <tr><td colSpan={7} className="py-20 text-center font-black text-slate-400 bg-slate-50 italic">এই সাইকেলে কোনো চিঠিপত্র তথ্য পাওয়া যায়নি।</td></tr>
            )}
                       {isAdmin && (
